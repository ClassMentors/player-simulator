...
var worker = proxyquire("../src/simulatePlayers", {
...
describe("simulatePlayer", function() {
    it("can read options", function() {
        var args = ["1", "2","test2=test2"];
        worker.setup(args);
        expect(worker.options['test2']).to.equal("test2");
    });

    /*
    // TODO: Figure out how to simulate logins. 
    it("can authenticate", function(done) {
        var args = ["1", "2","test2=test2"];
        expect(worker.options['test2']).to.equal("test2");

        worker.authenticateWithFirebase(done); 
        //console.log(worker.ref);
    ...
    */
    ...
    it("can load simulation data", function(done) {
      ...
      var callback = function(id){done();};

      worker.createNewPlayer("A", "player1", "NEED_TOKENS", callback);


    });

    it("can start workers", function() {
        worker.load_data({
            "test":"test"
        });
    });
    ...
    it("processes service responses", function() {
        //Todo - update to support new parser. 
    ...
    //Todo: Check for bad fccProfiles. Update to -1 rather than <!. Duplicate error first. 
